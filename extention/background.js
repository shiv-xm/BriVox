...
async function openReaderPage(op, pdfUrl) {
    try {
        ...
        console.warn("BRIVOX: failed to open reader page:", e?.message || e);
    }
}
    ...
    chrome.contextMenus.removeAll(() => {
        try {
            chrome.contextMenus.create({
                id: "brivox-summarize-selection",
                title: "Brivox: Summarize Selection",
                contexts: ["selection"]
            });
            chrome.contextMenus.create({
                id: "brivox-explain-selection",
                title: "Brivox: Explain Selection",
                contexts: ["selection"]
            });
            chrome.contextMenus.create({
                id: "brivox-translate-selection",
                title: "Brivox: Translate Selection",
                contexts: ["selection"]
            });
            chrome.contextMenus.create({
                id: "brivox-process-full",
                title: "Brivox: Process Full Document",
                contexts: ["page", "selection", "frame"]
            });
            chrome.contextMenus.create({
                id: "brivox-quiz-page",
                title: "Brivox: Quiz Me (Entire Page)",
                contexts: ["page"]
            });
        } catch (e) {}
    });
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
    ...
    if (info.menuItemId === "brivox-summarize-selection") {
        if (isViewer) {
            const pdfUrl = extractPdfUrlFromViewer(url) || url;
            return openReaderPage("summarize_full", pdfUrl);
        }
        if (tabId >= 0) return sendMessageToTab(tabId, { type: "BRIVOX_CONTEXT_ACTION", operation: "summarize" });
        return;
    }

    if (info.menuItemId === "brivox-explain-selection") {
        if (isViewer) {
            const pdfUrl = extractPdfUrlFromViewer(url) || url;
            return openReaderPage("explain", pdfUrl);
        }
        if (tabId >= 0) return sendMessageToTab(tabId, { type: "BRIVOX_CONTEXT_ACTION", operation: "explain" });
        return;
    }

    if (info.menuItemId === "brivox-translate-selection") {
        if (isViewer) {
            const pdfUrl = extractPdfUrlFromViewer(url) || url;
            return openReaderPage("translate", pdfUrl);
        }
        if (tabId >= 0) return sendMessageToTab(tabId, { type: "BRIVOX_CONTEXT_ACTION", operation: "translate" });
        return;
    }

    if (info.menuItemId === "brivox-process-full") {
        if (isViewer) {
            const pdfUrl = extractPdfUrlFromViewer(url) || url;
            return openReaderPage("summarize_full", pdfUrl);
        }
        if (tabId >= 0) {
            return sendMessageToTab(tabId, { type: "BRIVOX_CONTEXT_ACTION", operation: "process_full" });
        }
        return;
    }

    if (info.menuItemId === "brivox-quiz-page") {
        if (tabId < 0) return;
        ...
        try {
            ...
            if (!apiToken) {
                return toast(tabId, "Please log in from the Brivox popup first.");
            }
            ...
            if (res.status === 401) {
                ...
                return toast(tabId, "Unauthorized. Please log in again from the Brivox popup.");
            }
            if (!res.ok) {
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg?.type === "BRIVOX_AUTH_SIGNUP") {
        (async () => {
            try {
                const { username, password, fullName, path } = msg;
                if (!username || !password || !fullName) throw new Error("Full name, email and password are required.");
                ...
                const res = await fetch(url, {
                    ...
                    throw new Error("Cannot connect to backend. Make sure the server is running on " + backendUrl);
                });
    ...
    if (msg?.type === "BRIVOX_COMPARE_CONCEPT") {
        (async () => {
            try {
                ...
                if (!backendUrl) throw new Error("Backend URL not configured in Options.");
                if (!apiToken) throw new Error("Please log in from the Brivox popup.");
    ...
    if (msg?.type === "BRIVOX_AUTH_LOGOUT") {
        (async () => {
            await chrome.storage.sync.set({ apiToken: "", tokenExp: 0 });
            sendResponse({ ok: true });
        })();
    ...
    if (msg?.type === "BRIVOX_FETCH_ARRAYBUFFER" && msg.url) {
        (async () => {
            try {
    ...
    if (msg?.type === "BRIVOX_OPEN_READER") {
        (async () => {
            try {
...
chrome.runtime.onMessage.addListener((msg, _sender, _sendResponse) => {
    if (msg?.type === "BRIVOX_OPEN_ONBOARDING") {
        const url = chrome.runtime.getURL("pages/onboarding.html");
        chrome.tabs.create({ url }).catch(() => {});
    }
});
...
function toast(tabId, message) {
    try {
        chrome.tabs.sendMessage(tabId, { type: "BRIVOX_TOAST", message }, () => {
