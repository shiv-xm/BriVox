...
(function init() {
    ...
    let __bv_last_render_op = "";

    // Track AI path for microcopy in result panels
    let __bv_last_ai_path = ""; // "device" | "cloud" | ""
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
        ...
        if (changes.targetLang) {
            ...
            if (settings.mode !== "online-only") {
                // do not flip the __bv_prewarmed guard so we can re-send just for language updates
        ...
        if (changes.theme) {
            ...
            pgApplyTheme();
            try { __bv_toolbar_updateTheme?.(); } catch {}
        }
    });
    ...
    function prewarmAllApis() {
        try {
            ...
            window.postMessage({ type: "BRIVOX_AI_PREWARM", id: "prewarm_all", want }, "*");
        } catch {}
    }
...
//  Mark the device as ready when we see any AI events from the bridge.
    window.addEventListener("message", (ev) => {
        try {
            ...
            if (d.type === "BRIVOX_AI_READY" && d.ready) {
                __bv_deviceReady = true;
                return;
            }
            if (d.type === "BRIVOX_AI_PROGRESS" || d.type === "BRIVOX_AI_RESPONSE") {
                __bv_deviceReady = true;
            }
        } catch {}
    }, true);
    ...
    // Prewarm on-device
    let __bv_prewarmed = false;
    function prewarmLikely() {
        if (__bv_prewarmed) return;
        __bv_prewarmed = true;
    ...
    function probeDeviceReady(timeoutMs = 800) {
        return new Promise((resolve) => {
            ...
            const handler = (ev) => {
                ...
                const d = ev.data;
                if (!d || d.type !== "BRIVOX_AI_READY") return;
                window.removeEventListener("message", handler, true);
                finish(!!d.ready);
            };
            window.addEventListener("message", handler, true);
            try { window.postMessage({ type: "BRIVOX_AI_PING" }, "*"); } catch {}
            setTimeout(() => {
...
// Only cache TRUE; if probe fails, we retry next time.
    async function ensureOnDeviceReady({ retries = 2, timeout = 800, backoff = 300 } = {}) {
        if (__bv_deviceReady) return true;
        for (let attempt = 0; attempt <= retries; attempt++) {
            const ok = await probeDeviceReady(timeout + attempt * backoff);
            if (ok) { __bv_deviceReady = true; return true; }
        }
        return false;
    }
    ...
    let __bv_isMouseDown = false;
    document.addEventListener("mousedown", (e) => {
        const inOwnUi =
            (window.__bv_toolbar_root && window.__bv_toolbar_root.contains(e.target)) ||
            (__bv_fab_root && __bv_fab_root.contains(e.target));
        if (inOwnUi) return;
        __bv_isMouseDown = true;
        toolbar.hide();
    }, true);

    document.addEventListener("mouseup", () => {
        __bv_isMouseDown = false;
        setTimeout(() => {
    ...
    const __bv_loaders = new Map();
    chrome.runtime.onMessage.addListener((msg) => {
        // Hotkeys from background commands
        if (msg?.type === "BRIVOX_HOTKEY") {
        ...
        // Context menu relay
        if (msg?.type === "BRIVOX_CONTEXT_ACTION") {
        ...
        // Toast relay
        if (msg?.type === "BRIVOX_TOAST" && msg.message) {
            showToast(msg.message);
            return;
        }

        // Background-driven loader lifecycle
        if (msg?.type !== "BRIVOX_LOADING") return;
        const id = msg.requestId || "default";
        if (msg.action === "start") {
            try { __bv_loaders.get(id)?.close?.(); } catch {}
            const loader = createLoadingToast(msg.message || "Working…");
            __bv_loaders.set(id, loader);
            return;
        }
        const loader = __bv_loaders.get(id);
        if (!loader) return;
        if (msg.action === "set") loader.set(msg.message || "Working…");
        else if (msg.action === "success") { loader.success(msg.message || "Done"); __bv_loaders.delete(id); }
        else if (msg.action === "error") { loader.error(msg.message || "Error"); __bv_loaders.delete(id); }
        else if (msg.action === "close") { loader.close(); __bv_loaders.delete(id); }
    });
        ...
        try {
            if (looksLikePdfPage()) {
                ...
                chrome.runtime.sendMessage(
                    { type: "BRIVOX_OPEN_READER", op: "summarize_full", src: location.href },
                    () => {}
                );
        ...
        try {
            ...
            if (structured && Array.isArray(structured.bullets)) {
                ...
                if (wantAutoSearch && (!structured.citations || structured.citations.length === 0)) {
                    ...
                    try {
                        const found = await __bv_fetchSearchCitationsFromBackend(active.text);
    ...
    function sendCompareConcept(selectionText, pageUrl) {
        return new Promise((resolve) => {
            ...
            chrome.runtime.sendMessage(
                { type: "BRIVOX_COMPARE_CONCEPT", selectionText, pageUrl },
                (resp) => {
    ...
    async function quizSelectedText() {
        ...
        try {
            ...
            chrome.runtime.sendMessage({ type: "BRIVOX_OPEN_QUIZ", quizId }, () => {
                if (chrome.runtime.lastError) {
                    try {
    ...
    async function __bv_fetchSearchCitationsFromBackend(sourceText) {
        const resp = await persist("/api/v1/sources/find", {
            text: sourceText,
            sourceUrl: location.href,
            persona: settings.persona,
...
// Make sure your routing calls ensureOnDeviceReady per request:
    ...
    function aiOnDeviceWithPersona(operation, text, targetLang, persona, citeSources, progressCb) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const resHandler = (ev) => {
                if (ev.source !== window) return;
                const d = ev.data;
                if (!d || d.type !== "BRIVOX_AI_RESPONSE" || d.id !== id) return;
                __bv_deviceReady = true;
                if (finished) return;
                finished = true;
                cleanup();
            ...
            const progHandler = (ev) => {
                if (ev.source !== window) return;
                const d = ev.data;
                if (!d || d.type !== "BRIVOX_AI_PROGRESS" || d.id !== id) return;
                __bv_deviceReady = true;
            ...
            try {
                window.postMessage({ type: "BRIVOX_AI_REQUEST", id, operation, text, targetLang, persona, citeSources }, "*");
            } catch (e) {
    ...
    async function aiOnlineWithPersona(operation, text, targetLang, persona, citeSources, onProgress) {
        ...
        const resp = await __bv_persistWithRetry("/api/v1/ai", payload, { retries: 2, baseDelay: 900, maxDelay: 4500, onProgress });
    ...
    function persist(endpoint, payload) {
        return new Promise(resolve => {
            try {
                ...
                chrome.runtime.sendMessage(
                    { type: "BRIVOX_PERSIST", endpoint, payload },
                    (resp) => {
                        if (chrome.runtime.lastError) {
                            return resolve({ ok: false, error: chrome.runtime.lastError.message || "Message failed" });
                        }
    ...
    function showTranslationBubble(range, text) {
        ...
        bubble.className = "brivox-translation-bubble";
    ...
    function __bv_extractFirstJsonObject(text) {
...
// Normalize various schema shapes to { bullets: [], citations: [] }
    function __bv_normalizeStructured(obj) {
    ...
    function tryParseStructuredResult(text) {
        ...
        obj = __bv_extractFirstJsonObject(text);
        if (!obj) return null;

        // 2) Normalize to expected shape
        return __bv_normalizeStructured(obj); // null if not suitable
    }
    ...
    function showStructuredResultPanel({ bullets = [], citations = [] } = {}, opts = {}) {
        ...
        const topTitle = opts.title || friendlyTitle(__bv_last_render_op);
        ...
        __bv_makeCloudFooter?.(panel); // if you added the “Why cloud?” footer helper
    ...
    function showResultPanel(text) {
        ...
        const panel = document.createElement("div");
        const c = pgColors();
        panel.className = "brivox-result-panel";
        Object.assign(panel.style, {
            position: "fixed",
            right: "20px",
            bottom: "20px",
        ...
        __bv_makeCloudFooter(footerWrap);
    ...
    function showCategoriesBubbleWithFallback(categories) {
        ...
        const bubble = document.createElement("div");
        const c = pgColors();
        bubble.className = "brivox-categories-bubble";
    ...
    function showSuggestionsPanel(suggestions) {
        ...
        const panel = document.createElement("div");
        const c = pgColors();
        panel.className = "brivox-suggestions-panel";
    ...
    (function __bv_empty_state_bootstrap() {
...
// Wherever we receive on-device progress events, set this flag if we see "Downloading model".
    ...
    function __bv_getWhyCloudText() {
        ...
        try {
            ...
            // Device readiness
            if (!__bv_deviceReady) lines.push("On‑device APIs not ready yet in this tab.");
            if (__bv_modelsDownloading) lines.push("On‑device model is still downloading.");
            // Fallback generic
    ...
    function __bv_attachTooltip(anchorEl, text) {
        let tip = null;
        function show() {
            if (tip) return;
    ...
    function __bv_makeCloudFooter(containerEl) {
        try {
            ...
            const pathStr = __bv_last_ai_path === "device" ? "On‑device" : (__bv_last_ai_path === "cloud" ? "Cloud" : "Auto");
            ...
            __bv_attachTooltip(info, __bv_getWhyCloudText());
    ...
    function pgColors() {
        ...
        if (mode === "light") {
            return {
                ...
                muted: "#4d5b78",
                bg: "rgb(239, 231, 231)",
                surface: "rgb(249, 244, 244)",
                border: "rgba(0,0,0,0.14)",
                border2: "rgba(0,0,0,0.08)",
                focus: "#1d4ed8",
                accent: "#2563eb",
                panelBg: "rgb(239, 231, 231)",
                toolbarBg: "rgb(239, 231, 231)",
                toastBg: "rgb(239, 231, 231)",
                btn: "rgb(249, 244, 244)",
                btnHover: "rgb(230, 220, 220)"
            };
        }
        return {
            text: "rgb(32, 29, 29)",
    ...
    function pgInjectStyles() {
        if (document.getElementById("brivox-a11y-anim")) return;
        const style = document.createElement("style");
        style.id = "brivox-a11y-anim";
        style.textContent = `
    ...
    function pgApplyTheme() {
        const c = pgColors();
        const root = document.getElementById("brivox-fab");
        if (root) {
    ...
    // FAB
    let __bv_fab_root = null;
    let __bv_fab_observer = null;

    function ensureFloatingButton() {
        try { if (window.top !== window.self) return; } catch {}
        if (__bv_fab_root && document.documentElement.contains(__bv_fab_root)) return;
        ...
        menu.setAttribute("aria-label", "Brivox actions");

        const btn = document.createElement("button");
        btn.id = "brivox-fab-btn";
        btn.title = "Brivox";
        btn.setAttribute("aria-label", "Open Brivox menu");
        ...
        function mbtn(label, icon, onClick, shortcut) {
            ...
            b.className = "brivox-fab-item pg-focus pg-anim-pop";
        ...
        try {
            if (__bv_fab_observer) __bv_fab_observer.disconnect();
            __bv_fab_observer = new MutationObserver(() => {
                if (!settings.showFloatingButton) return;
                if (!document.documentElement.contains(__bv_fab_root)) {
                    __bv_fab_root = null;
                    ensureFloatingButton();
                }
            });
            __bv_fab_observer.observe(document.documentElement, { childList: true });
        } catch {}
    ...
    function removeFloatingButton() {
        if (__bv_fab_observer) {
            try { __bv_fab_observer.disconnect(); } catch {}
            __bv_fab_observer = null;
        }
        if (__bv_fab_root) {
            try { __bv_fab_root.__bv_cleanup?.(); } catch {}
            try { __bv_fab_root.remove(); } catch {}
            __bv_fab_root = null;
        }
    }
    ...
    function createToolbar() {
        ...
        const root = document.createElement("div");
        root.className = "brivox-toolbar pg-anim-fade";
        root.setAttribute("role", "toolbar");
        root.setAttribute("aria-label", "Brivox selection actions");
        ...
        function tbtn(id, label, iconName, shortcut) {
            const b = document.createElement("button");
            b.type = "button";
            b.className = "brivox-btn pg-focus";
        ...
        document.documentElement.appendChild(root);
        window.__bv_toolbar_root = root;

        // Live theme refresh
        window.__bv_toolbar_updateTheme = () => {
            const cc = pgColors();
            ...
            root.querySelectorAll(".brivox-btn").forEach(b => {
    ...
    function injectPageBridge() {
        try {
            ...
            if (document.getElementById("brivox-bridge-script")) return;
            if (document.documentElement.querySelector(`script[src="${src}"]`)) return;
            const script = document.createElement("script");
            script.id = "brivox-bridge-script";
            script.src = src;
...
// Jittered sleep
    function __bv_sleep(ms) { return new Promise((r) => setTimeout(r, ms)); }
    function __bv_backoffDelay(attempt, base = 800, cap = 5000) {
...
// Normalize/clean backend error payloads (including your <EOL> shape)
    function __bv_parseBackendErr(resp) {
    ...
    function __bv_isTransientBackendError(err) {
        const s = err.status;
    ...
    function __bv_friendlyCloudError(err) {
        const s = err.status;
        const m = (err.message || "").toLowerCase();
...
// Retry wrapper around persist()
    async function __bv_persistWithRetry(endpoint, payload, { retries = 2, baseDelay = 900, maxDelay = 4500, onProgress } = {}) {
        let lastErr = null;
        for (let attempt = 0; attempt <= retries; attempt++) {
            const resp = await persist(endpoint, payload);
            if (resp?.ok) return resp;

            const err = __bv_parseBackendErr(resp);
            lastErr = err;

            if (attempt < retries && __bv_isTransientBackendError(err)) {
                const wait = __bv_backoffDelay(attempt, baseDelay, maxDelay);
                try { onProgress?.(`Cloud busy • retrying in ${(wait / 1000).toFixed(1)}s (${attempt + 1}/${retries + 1})`); } catch {}
                await __bv_sleep(wait);
                continue;
            }
            break; // non-transient or out of retries
        }
        return { ok: false, error: __bv_friendlyCloudError(lastErr || {}), status: lastErr?.status || 0, data: { code: lastErr?.code || "server_error", message: lastErr?.message || "" } };
    }

    // Dev hooks
    window.__bv_showSuggestions = function(example) {
        const arr = normalizeSuggestionsShape(example);
        if (arr.length) showSuggestionsPanel(arr);
    };
    window.__bv_showCategories = function(obj) {
