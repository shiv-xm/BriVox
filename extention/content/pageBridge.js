...
// - BRIVOX_AI_PING     => replies with BRIVOX_AI_READY { ready: boolean }
// - BRIVOX_AI_PREWARM  => (optional) asks bridge to pre-create local sessions
// - BRIVOX_AI_REQUEST  => replies with BRIVOX_AI_RESPONSE { ok, result|error, id }
// - Emits BRIVOX_AI_PROGRESS { id, message } for user-friendly loaders.
//
// IMPORTANT: Do NOT reference chrome.* APIs here. This file executes in page context.

(function () {
    if (window.__BRIVOX_BRIDGE_LOADED__) return;
    window.__BRIVOX_BRIDGE_LOADED__ = true;

    const NS_REQ   = "BRIVOX_AI_REQUEST";
    const NS_RES   = "BRIVOX_AI_RESPONSE";
    const NS_PROG  = "BRIVOX_AI_PROGRESS";
    const NS_PING  = "BRIVOX_AI_PING";
    const NS_READY = "BRIVOX_AI_READY";
    const NS_PREWARM = "BRIVOX_AI_PREWARM";
