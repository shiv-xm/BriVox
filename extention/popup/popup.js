(function () {
    ...
    loginForm?.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            const resp = await new Promise((resolve) => {
                chrome.runtime.sendMessage({ type: "BRIVOX_AUTH_LOGIN", username, password }, (r) => {
    ...
    signupForm?.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            const resp = await new Promise((resolve) => {
                chrome.runtime.sendMessage({ type: "BRIVOX_AUTH_SIGNUP", fullName, username, password }, (r) => {
                    if (chrome.runtime.lastError) resolve({ ok: false, error: chrome.runtime.lastError.message }); else resolve(r);
                });
            });
    ...
    logoutBtn?.addEventListener("click", async () => {
        ...
        try {
            const resp = await new Promise((resolve) => {
                chrome.runtime.sendMessage({ type: "BRIVOX_AUTH_LOGOUT" }, (r) => {
                    if (chrome.runtime.lastError) resolve({ ok: false, error: chrome.runtime.lastError.message }); else resolve(r);
                });
            });
    ...
    readingModeBtn?.addEventListener("click", async () => {
        try {
            await new Promise((resolve) => chrome.runtime.sendMessage({ type: "BRIVOX_OPEN_READER", op: "summarize_full" }, () => resolve()));
  ...
  `;
    ...
    document.addEventListener("click", (e) => {
        if (e.target && e.target.id === "openOnboarding") {
            chrome.runtime.sendMessage({ type: "BRIVOX_OPEN_ONBOARDING" });
        }
    });
